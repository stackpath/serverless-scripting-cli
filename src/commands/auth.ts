...
import { Auth as AuthService } from '../api/services/auth';
import { Log, LogLevel } from '../api/services/log';
...
export default class Auth extends Command {
  ...
  public static flags: flags.Input<any> = {
    client_id: flags.string({ char: 'c', description: 'StackPath Client ID used to authenticate with' }),
    client_secret: flags.string({ char: 's', description: 'StackPath Client Secret used to authenticate with' }),
    force: flags.boolean({
      char: 'f',
      description: 'Set this to always overwrite current credential file, defaults to false.',
      default: false,
    }),
    help: flags.help({ char: 'h' }),
    verbose: flags.boolean({ char: 'v', description: 'Turns on verbose logging. Defaults to false', default: false }),
  };
  ...
  public async run() {
    ...
    if (AuthService.credentialsFileExists() && !this.parsedFlags.force) {
      const overwriteFile =
        await cliUx.prompt('It seems a credentials file already exists. Continuing will overwrite its values. Continue? y/n');
    ...
    const clientID = this.parsedFlags.client_id || await cliUx.prompt('StackPath Client ID');
    const clientSecret = this.parsedFlags.client_secret || await cliUx.prompt('StackPath Client Secret', {
      type: 'hide',
    });
