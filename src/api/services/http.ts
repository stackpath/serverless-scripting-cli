...
/*
 * Namespace performing all HTTP tasks.
 */
export namespace Http {
   ...
   */
  export async function request(method: string, resource: string, body?: any, throwErrors: boolean = true): Promise<NodeFetchResponse> {
    try {
      ...
      throw new Error(`An error occurred when connecting to StackPath host ${STACKPATH_HOST}.`);
    }
  }
   ...
   */
  export async function handleResponseError(response: NodeFetchResponse, throwErrors: boolean = true): Promise<boolean> {
    if (response.status !== 200) {
      const error = await response.clone()
                                  .json();
