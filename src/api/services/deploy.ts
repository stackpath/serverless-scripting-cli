...
export class Deploy {
    ...
    public async deployScripts(options?: { force: boolean }) {
        ...
        if (!fs.existsSync(configFile)) {
            throw new Error(`The current directory does not contain the required ${STACKPATH_CONFIGFILE_PATH} file.`);
        }
        ...
        try {
            await this.configuration.scripts.reduce(async (previousPromise, nextScript, index) => {
                await previousPromise;
                return this.handleScriptUpload(nextScript, index);
            }, Promise.resolve());

            Log.logVerbose(`Saving new configuration file to ${STACKPATH_CONFIGFILE_PATH}`);
            fs.writeFileSync(configFile, JSON.stringify(this.configuration, undefined, 4));
     ...
     */
    private async handleScriptUpload(script: IConfigurationScript, index: number) {
        ...
        const scriptContentBase64 = Buffer
            .from(scriptContent)
            .toString('base64');
        ...
        const siteId = (script.hasOwnProperty('site_id') && script.site_id !== '') ? script.site_id : this.configuration.site_id;
        if (script.hasOwnProperty('id') && script.id !== '') {
            Log.logVerbose(`Updating script ${script.name} with id ${script.id} to site ${siteId}.`);

            const response = await Http.request(
                'PATCH',
                `/cdn/v1/stacks/${this.configuration.stack_id}/sites/${siteId}/scripts/${script.id}`,
                body,
                false,
            );

            const json = await response.clone()
                .json();

            if (response.status === 404 && json.code === 5) { // code 5 means Site script does not exist.
        ...
        if (!script.hasOwnProperty('id') || script.id === '') {
            ...
            Log.logVerbose(`HTTP Response for script ${script.name}: ${response.status}`);
     ...
     */
    private async updateScriptWithResponse(script: IConfigurationScript, response: NodeFetchResponse, index: number) {
        if (response !== undefined) {
            const json = await response.clone()
                .json();
            if (json) {
                Log.logVerbose(`Saving id for created script ${script.name} to ${json.script.id}`);
